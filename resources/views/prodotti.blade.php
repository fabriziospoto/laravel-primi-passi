...
$cards = json_decode($data,true); //converto json
$lunghe = [];  //creo tre array vuoti per tipi di pasta
$corte = [];
$cortissime = [];

foreach ($cards as $card) {  // ciclo foreach con tre condizioni per far scorrere in base al tipo
    if($card['tipo'] == 'lunga'){
        $lunghe[] = $card; //pusho nell'array corrispondente
    }
    elseif($card['tipo'] == 'corta'){
        $corte[] = $card; //pusho nell'array corrispondente
    }
    elseif($card['tipo'] == 'cortissima'){
        $cortissime[] = $card; //pusho nell'array corrispondente
    }
}
